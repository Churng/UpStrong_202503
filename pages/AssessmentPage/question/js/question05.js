...
$(document).ready(function () {
  ...
  $(".next").on("click", function () {
    if (step == "01") {
      ...
      const dateBoxValues = $(".date-box .past-box").map((_, el) => $(el).text().trim()).get();``
      ...
      for (i=0; i<lengthOfList; i++){
        ...
        const pastValues = $(`.table-box[data-past=${i}] .past-box`).map((_, el) => $(el).text().trim()).get();//回傳過來的目標值
  ...
  $(".button-box button").on("click", function () {
    ...
    if (testdata.text()) {
      ...
      targetScore =
        ...
        Number($("[name='level']:checked").val() - Number(testdata.text()));
